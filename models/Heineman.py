...
class Heineman(Model):
    ...
    def get_category_transition_matrices(self):
        ...
        normalized_transition_matrices = np.zeros(
            (self.config["cv"], self.num_categories, self.num_categories)
        )
        if self.config["fitting"] == "group":
            ...
            transition_matrix = np.zeros((self.num_categories, self.num_categories))
            self.data["categories"] = self.data["response"].map(
                self.response_to_category
            )
            self.data["previous_categories"] = self.data.groupby("pid")[
                "categories"
            ].shift()
    ...
    def get_category_cue(self, response, previous_response):
        ...
        if len(cats_intersection) != 0:
            return max(
                [self.cat_trans[self.split_ind, c, c] for c in cats_intersection]
            )
        else:
            return max(
                [
                    self.cat_trans[self.split_ind, c1, c2]
                    for c1 in cats_prev
                    for c2 in cats_curr
                ]
            )
    ...
    def get_nll(self, seq, weightsfromarg=None):
        ...
        try:
            ...
            cat_trans_terms = torch.stack(
                [
                    torch.tensor(
                        [self.get_category_cue(r_, r) for r_ in self.unique_responses],
                        dtype=float,
                        device=device,
                    )
                    for r in seq[1:-1]
                ]
            ).to(device=device)  # shape: (len_seq - 2, num_resp)
