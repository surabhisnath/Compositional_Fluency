...
class Hills(Model):
    ...
    def create_models(self):
        ...
        self.models = {
            subclass.__name__: subclass(self)
    ...
    def get_nll(self, seq, weightsfromarg=None):
        ...
        sim_terms_mask = torch.ones(
            len(seq) - 2, dtype=torch.int8, device=device
        ).unsqueeze(1)  # Default: all ones
        if self.dynamic:
            if self.dynamic_cat:
                if seq[-1] == "":  # --- Simulation mode: predict next response
                    prev_resp = seq[-2]
                    prev_cat = set(self.response_to_category[prev_resp])
                    sim_terms_mask = torch.tensor(
                        [
                            bool(prev_cat & set(self.response_to_category[r]))
                            for r in self.unique_responses
                        ],
                        dtype=torch.int8,
                        device=device,
                    ).unsqueeze(0)  # shape: (1, num_resp)
                else:
                    sim_terms_mask = torch.tensor(
                        [
                            bool(
                                set(self.response_to_category[seq[i]])
                                & set(self.response_to_category[seq[i - 1]])
                            )
                            for i in range(2, len(seq))
                        ],
                        dtype=torch.int8,
                        device=device,
                    ).unsqueeze(1)  # shape: (len(seq) - 2, 1)
...
class OneCueStaticLocalWeighted(Hills, nn.Module):
    ...
    def get_nll(self, seq, weightsfromarg=None):
        ...
        prev_embeddings = torch.stack(
            [torch.tensor(self.embeddings[r], device=device) for r in seq[1:-1]]
        ).to(device=device)  # Shape: (L, D) where L = len(seq) - 2
        all_embeddings = torch.stack(
            [
                torch.tensor(self.embeddings[r], device=device)
                for r in self.unique_responses
            ]
        )  # Shape: (N, D) where N = num unique responses
        all_dots = (
            prev_embeddings.unsqueeze(1) * all_embeddings.unsqueeze(0)
        ).float()  # Shape: (L, N, D)
...
class OneCueStaticLocalWeightedActivity(Hills, nn.Module):
    ...
    def get_nll(self, seq, weightsfromarg=None):
        ...
        logits = torch.einsum(
            "lnd,d->ln", all_dots, weightstouse[: self.num_weights // 2]
        ) + torch.einsum(
            "nd,d->n",
            all_embeddings.to(torch.float),
            weightstouse[self.num_weights // 2 :],
        ).unsqueeze(0)  # Shape: (L, N)
...
class CombinedCueStaticWeightedActivity(Hills, nn.Module):
    ...
    def get_nll(self, seq, weightsfromarg=None):
        ...
        logits = (
            ...
            + torch.einsum(
                "lnd,d->ln", all_dots, weightstouse[1 : 1 + self.num_weights // 2]
            )
            + torch.einsum(
                "nd,d->n",
                all_embeddings.to(torch.float),
...
class CombinedCueStaticWeighted(Hills, nn.Module):
    ...
    def get_nll(self, seq, weightsfromarg=None):
        ...
        logits = weightstouse[0] * self.d2ts(self.freq).unsqueeze(0) + torch.einsum(
            "lnd,d->ln", all_dots, weightstouse[1:]
        )  # Shape: (L, N)
