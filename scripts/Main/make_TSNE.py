...
def make_TSNE(
    embeddings, responses, clusters=None, show_responses=False, feature_name=None
):
    """Plot and save a TSNE scatter for the provided embeddings."""
    assert len(embeddings) == len(responses), (
        "Embeddings and responses must have the same length"
    )

    tsne = TSNE(
        n_components=2, perplexity=10, max_iter=3000, random_state=42, metric="hamming"
    )
    ...
    plt.savefig(
        f"../../plots/Figure1/tsne_plot_{feature_name}.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
if __name__ == "__main__":
    ...
    features = {
        corrections.get(k, k): np.array(
            [1 if values.get(f, "").lower()[:4] == "true" else 0 for f in feature_names]
        )
    ...
    make_TSNE(
        embeddings,
        responses,
        embeddings[:, feature_name_to_ind["feature_Is mammal"]],
        False,
        "Is mammal",
    )
    make_TSNE(
        embeddings,
        responses,
        embeddings[:, feature_name_to_ind["feature_Is domesticated"]],
        False,
        "Is domesticated",
    )
    make_TSNE(
        embeddings,
        responses,
        embeddings[:, feature_name_to_ind["feature_Is bird"]],
        False,
        "Is bird",
    )
    make_TSNE(
        embeddings,
        responses,
        embeddings[:, feature_name_to_ind["feature_Is migratory"]],
        False,
        "Is migratory",
    )
