...
model_name_to_model_print = json.load(
    open("../../files/model_name_to_model_print.json", "r")
)
...
modelnames = [model_name_to_model_print[m] for m in modelNLLs]
colors = [model_name_to_color[m] for m in modelNLLs]
...
plt.ylabel("Sum Test NLL (over 5 folds)")
plt.tight_layout()
