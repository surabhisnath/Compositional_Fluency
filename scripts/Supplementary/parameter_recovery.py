...
weight_names = [
    ...
    "Is primate",
    "Is jungle animal",
    "Is non-jungle animal",
    "Is feline",
    "Is canine",
    "Is subspecies of an animal",
    "Is carnivore",
    "Is herbivore",
    "Is omnivore",
    "Is larger in size compared to other animals",
    "Is smaller in size compared to other animals",
    "Is average size compared to other animals",
    "Is warm-blooded",
    "Is cold-blooded",
    "Is a predator",
    "Is prey for larger animals",
    "Is a parasite",
    "Is a host for parasites",
    "Is nocturnal",
    "Is diurnal",
    "Has fur",
    "Has feathers",
    ...
    "Has wings",
    "Has tail",
    "Has less than four limbs",
    "Has exactly four limbs",
    "Has more than four limbs",
    "Has stripes",
    "Has spots",
    "Has mane",
    "Has crest",
    ...
    "Can camouflage",
    "Can hibernate",
    "Can be trained or tamed by humans",
    "Is found in zoos",
    "Lives in water",
    "Lives in trees",
    "Lives underground",
    ...
    "Is native to Europe",
    "Lives in Arctic/far North",
    "Is found in deserts",
    "Is found in forests",
    "Is found in oceans",
    "Is found in grasslands",
    ...
    "Is flightless",
    "Has webbed feet",
    "Is known for intelligence",
    "Is a scavenger",
    "Is territorial",
    "Is endangered",
    "Is bioluminescent",
    "Is capable of parental care",
    "Is a pollinator",
    "Can tolerate extreme temperatures",
    "Exhibits seasonal color changes",
    "Is active during dawn or dusk (crepuscular)",
    "Produces pheromones for communication",
    "Lives symbiotically with other species",
    "Is bi-parental (both parents care for offspring)",
    "Displays mating rituals",
    "Has specialized courtship behavior",
    "Exhibits territorial marking",
    "Is associated with mythology or folklore",
    "Exhibits altruistic behavior",
    "Is a keystone species in its ecosystem",
    "Can regenerate body parts",
    "Is raised in captivity or farms",
    "Has unique reproductive strategies (e.g., asexual reproduction)",
    "Hibernates during winter",
    "Has a role in biological pest control",
    "Has distinct seasonal breeding cycles",
    "Forms a symbiotic relationship with plants (e.g., pollination)",
    "Uses specific vocalizations to communicate",
    "Is a flagship species (conservation symbol)",
    "Displays warning coloration",
]
...
for i in range(1, 11):
    weights = (
        pk.load(open(f"../../fits/parameter_recovery/fakeweights{i}.pk", "rb"))
        .cpu()
        .numpy()
    )
...
for i in range(0, 11):
    weights1 = (
        pk.load(
            open(
                f"../../fits/parameter_recovery/freqweightedhsactivity_fits_gpt41_paramrecovery_{i}_1.pk",
                "rb",
            )
        )[f"weights_fold1_paramrecovery_{i}_1"]
        .cpu()
        .numpy()
    )
    weights2 = (
        pk.load(
            open(
                f"../../fits/parameter_recovery/freqweightedhsactivity_fits_gpt41_paramrecovery_{i}_2.pk",
                "rb",
            )
        )[f"weights_fold1_paramrecovery_{i}_2"]
        .cpu()
        .numpy()
    )
    weights3 = (
        pk.load(
            open(
                f"../../fits/parameter_recovery/freqweightedhsactivity_fits_gpt41_paramrecovery_{i}_3.pk",
                "rb",
            )
        )[f"weights_fold1_paramrecovery_{i}_3"]
        .cpu()
        .numpy()
    )

    recovered_weights_means.append((weights1 + weights2 + weights3) / 3)
    recovered_weights_se.append(
        np.std([weights1, weights2, weights3], axis=0) / np.sqrt(3)
    )
...
plt.tight_layout()
plt.savefig(
    f"../../plots/Supplementary/parameter_recovery/0_{weight_names[0].replace('/', ' or ')}_recovery.png",
    bbox_inches="tight",
    dpi=300,
)
plt.close()

results.append({"Feature": weight_names[0], "R2_freq": r2_freq, "RMSD_freq": rmsd_freq})
...
for w in np.arange(1, 139):
    ...
    ax[0].set_xlim(
        min(true_weights[:, w].min(), recovered_weights_means[:, w].min()) - 0.1,
    ...
    offset = 138
    r2_act = identity_r2(
        true_weights[:, w + offset], recovered_weights_means[:, w + offset]
    )
    rmsd_act = identity_rmsd(
        true_weights[:, w + offset], recovered_weights_means[:, w + offset]
    )
    ax[1].scatter(
        true_weights[0, w + offset],
        recovered_weights_means[0, w + offset],
        color="red",
        label="True",
        edgecolor="k",
        s=60,
    ...
    for i in range(1, 11):
        color = cmap(i / 10)
        ax[1].scatter(
            true_weights[i, w + offset],
            recovered_weights_means[i, w + offset],
            color=color,
            label=f"Var {i}",
            edgecolor="k",
            s=60,
    ...
    ax[1].set_xlim(
        min(
            true_weights[:, w + offset].min(),
            recovered_weights_means[:, w + offset].min(),
        )
        - 0.1,
        max(
            true_weights[:, w + offset].max(),
            recovered_weights_means[:, w + offset].max(),
        )
        + 0.1,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"../../plots/Supplementary/parameter_recovery/{w}_{weight_names[w].replace('/', ' or ')}_recovery.png",
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
